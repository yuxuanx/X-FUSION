function [shortestPaths, totalCosts] = kShortestPath_any(netCostMatrix, source, destination, k_paths)
...
if source > size(netCostMatrix,1) || destination > size(netCostMatrix,1)
    ...
    %---------------------INITIALIZATION---------------------
    k=1;
    [cost,path,~] = BFMSpathwrap(sparse(netCostMatrix), source, destination); %[cost,path,~] = graphshortestpath(sparse(netCostMatrix), source, destination,'Method','Bellman-Ford');
    ...
    if isempty(path)
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1   %index_dev_vertex is index in P_ of deviation vertex
                ...
                [c,dev_p,notusedatall2] = BFMSpathwrap(sparse(temp_netCostMatrix), P_(index_dev_vertex), destination); %graphshortestpath(sparse(temp_netCostMatrix), P_(index_dev_vertex), destination,'Method','Bellman-Ford');
                if ~isempty(dev_p)
